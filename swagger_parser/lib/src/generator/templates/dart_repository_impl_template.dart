...
String dartRepositoryImplClientTemplate({
...
${dartImports(imports: {'${name}Repository',})}
${dartImports(imports: {'${name}RemoteDataSource',},pathPrefix: '../remote/datasource/')}


@Injectable(as: ${name}Repository)
...
String _parseDataSourceFunctionBody(UniversalRequest request){
  final sb = StringBuffer(
      '''
    try {
      final result =  await _remoteDataSource.${request.name}(
'''
  );
  final sortedByRequired = List<UniversalRequestType>.from(
    request.parameters.sorted((a, b) => a.type.compareTo(b.type)),
